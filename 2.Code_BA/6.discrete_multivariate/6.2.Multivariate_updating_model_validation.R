...
mean_vector_function <- function(parameters, BuM_data) {
  ...
  #mean_vector <- c(0.879, 0.853,0.716,0.642,1.300,1.389,1.679,1.805,1.932)
  return(mean_vector)
...
update_list_multivariate <- list()


for(j in 1:nrow(BuM_data_original)) {
  ...
  #update year for k-cross validation 
  ...
  for (i in 1:nrow(optimized_correlation_matrix)) {
    ...
    for (p in 1:ncol(optimized_correlation_matrix)) {
      
      optimized_correlation_matrix[i,p] <- optimized_covariance_matrix[i,p] / (sd_vector[i] * sd_vector[p])
  ...
  #conditioning:
  for (i in (as.numeric(nrow(optimized_covariance_matrix)) : 2)) {
  ...
  #reformat distribution over time matrix 
  ...
  #store distribution over time matrix in list 
  update_list_multivariate[[j]] <- distribution_over_time
  
  
  #validation 
  
  for (y in 1 : (length(updating_data_year) - 1)) {
    
    score_temp <- -1*logs_norm(updating_data_year[1], mean = distribution_over_time[2, y + 1], sd = distribution_over_time[3, y + 1])
    
    
    validation_data[j, y] <- score_temp
...
names(update_list_multivariate) <- -1*(rownames(BuM_data_original))


#calculation of mean for every forecast horizon 
...
average_validation_score_multivariat <- sum / (nrow(validation_data) * (ncol(validation_data)-1))



validation_data_multivariat <- as.data.frame(validation_data) 


write_xlsx(validation_data_multivariat, "8.validation+comparison_discrete_models/validation_data_multivariat.xlsx")
